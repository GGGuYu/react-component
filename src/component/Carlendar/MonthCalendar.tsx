...
const renderDays = (days:Array<InfoOfDate>) => {
    ...
    for(let i = 0;i < 6;i++){
        ...
        for(let j = 0;j < 7;j++){
            const item = days[i*6+j];
            //每个小cell装进这一行
            row.push(
                <div className={"calendar-month-body-cell " + (item.isCurrentMonth ? 'calendar-month-body-cell-current' : '')}>
                    {item.date.date()}
                </div>
            )
        }
        //每一行套一个样式，因为里面要flex,然后再装进rows里面，其实分rows也行的
        rows.push(<div className="calendar-month-body-row">{row}</div>);
    }
    return rows;
}
